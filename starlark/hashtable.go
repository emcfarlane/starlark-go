...
func (d *OrderedStringDict) append(h uint32, k string, v Value) error {
	if d.frozen {
		return fmt.Errorf("cannot mutate frozen hash table")
	}
	if d.itercount > 0 {
		return fmt.Errorf("cannot mutate hash table during iteration")
	}
	if d.table == nil {
		d.init(1)
	}
...
func (d *OrderedStringDict) freeze() {
	if !d.frozen {
		d.frozen = true
...
func NewOrderedStringDict(d StringDict) *OrderedStringDict {
...
func (d *OrderedStringDict) Set(k string, v Value) error {
	if d.frozen {
		return fmt.Errorf("cannot mutate frozen hash table")
	}
	...
	return NoSuchAttrError(fmt.Sprintf("unknown key %s", k))
}
